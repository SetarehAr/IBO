...
def objective_function(params):
    ...
    ImportanceTraining(model,presample=float(B)).fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size),validation_data=(x_test, y_test),epochs=200, verbose=1,batch_size=batch_size,callbacks=callbacks,steps_per_epoch=int(np.ceil(float(len(x_train)) / batch_size)))
