...
def objective_function(params,s):
    ...
    # Train the model with IS
    ImportanceTraining(model,presample=float(B)).fit_generator(datagen.flow(train_subset, train_targets_subset, batch_size=batch_size),validation_data=(x_test, y_test),epochs=nb_epochs, verbose=1,batch_size=batch_size,callbacks=callbacks,steps_per_epoch=int(np.ceil(float(len(train_subset)) / batch_size)))
